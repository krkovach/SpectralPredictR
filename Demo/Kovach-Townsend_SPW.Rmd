...
- jump corrected, unit vector normalized, and ) will be available, though the model application will be with the vector normalized data (as the models were built using vector normalized data).
...
## Plots
Following are some methods to plot the data you've extracted.
...
This is a grouped manual outlier detection for samples with replicates.  It vector normalizes the spectra, and uses a sample and replicate value (extracted in this case from the file nomenclature) to display them separately dd

```{r Grouped Outlier Detection,message=F}
